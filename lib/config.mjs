...
export function loadConfig(argv = process.argv.slice(2)) {
  ...
  const config = {
    ...
    dataDir,
    // 屏幕截图归档目录（screen-tool 插件写入处，/shots 画廊读取处）。
    // 默认与插件一致：<DSH_HOME>/screenshots
    screenshotsDir: resolve(firstDefined(
      env.DSH_PHONE_AGENT_SCREENSHOTS_DIR, fileConfig.screenshotsDir,
      resolve(env.DSH_HOME ?? resolve(homedir(), '.dsh'), 'screenshots'),
    )),
    ...
    // 项目根（模块化组件解析自身资源/二进制时使用）
    projectRoot: PROJECT_ROOT,
    // 直播模块（自建帧流：屏幕 + 摄像头，MJPEG over HTTP）
    live: normalizeLive(fileConfig.live, env.DSH_PHONE_AGENT_LIVE),
  }
...
/**
 * 规范化 live 直播模块配置（默认全部开，最小化配置心智负担）。
 * @param {*} raw  config.json 中的 live 段
 * @param {string|undefined} envEnabled  环境变量 DSH_PHONE_AGENT_LIVE（'0' 禁用）
 */
function normalizeLive(raw = {}, envEnabled) {
  return {
    enabled: boolField(envEnabled, raw.enabled, true),
    // ffmpeg 可执行文件（相对项目根）；portable 静态构建放 tools/ffmpeg/bin/
    ffmpegPath: raw.ffmpegPath || 'tools/ffmpeg/bin/ffmpeg.exe',
    screen: {
      fps: intField(undefined, raw.screen?.fps, 5),
      width: intField(undefined, raw.screen?.width, 1280),
      quality: intField(undefined, raw.screen?.quality, 5),
    },
    camera: {
      // 'auto' = 枚举后取第一个视频设备；或写死 dshow 设备名
      device: raw.camera?.device ?? 'auto',
      fps: intField(undefined, raw.camera?.fps, 10),
      width: intField(undefined, raw.camera?.width, 640),
      quality: intField(undefined, raw.camera?.quality, 5),
    },
  }
}
