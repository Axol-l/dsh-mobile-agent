...
窗口截图使用 **PrintWindow** 抓取窗口自身内容（即使被其他窗口遮挡也能截
对窗口本体，而非屏幕上的遮挡内容）；对硬件加速/独占全屏窗口（PrintWindow
返回全黑）自动回退屏幕区域截图。结果带 `method` 字段
（`print-window` / `screen-region`）。
...
网关会在转发的 dsh web 主文档中注入一个右下角悬浮窗（`lib/proxy.mjs` 的
`SCREEN_WIDGET_SCRIPT`）：每 8 秒轮询 `/shots/list.json`，**最新截图直接
显示在页面角落**，不用打开画廊：

- 点击小窗任意位置（含标题栏）→ 打开 `/shots` 画廊看大图；
- 点右上角 × 关闭（本会话内保持隐藏，轮询不再恢复）；
- 桌面直连 `127.0.0.1:3080`（不走网关）时 `/shots` 不存在，悬浮窗自动隐藏；
- 改动悬浮窗行为后，重启网关 + 刷新页面生效。
...
- 窗口截图用 PrintWindow 抓窗口本体，被遮挡也能截对；仅对硬件加速/
  独占全屏（如部分游戏、视频播放器全屏）回退屏幕区域截图，此时若该
  区域被遮挡会截到遮挡内容——请改用桌面全屏截图。
- 最小化窗口无法截图（PrintWindow 返回空内容），会明确报错。
- 多显示器：桌面模式为所有显示器拼接图；窗口模式按窗口自身内容抓取。
